            ...
            <div class="p-5 space-y-3 md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-4 md:space-y-0">
                ...
                <?php if (!empty($doctors)): ?>
                    <?php foreach ($doctors as $doctor): ?>
                        ...
                        <div onclick="window.location.href='<?= base_url('patient/chat') ?>'"
                            class="bg-white rounded-2xl p-4 shadow-sm card-hover cursor-pointer border border-gray-50 <?= $doctor['status'] == 'offline' ? 'opacity-80' : '' ?>">
                            <div class="flex items-start">
                                <div class="w-14 h-14 <?= $doctor['status'] == 'online' ? 'bg-blue-100' : 'bg-gray-100' ?> rounded-2xl flex items-center justify-center mr-4 flex-shrink-0">
                                    <svg class="w-8 h-8 <?= $doctor['status'] == 'online' ? 'text-blue-600' : 'text-gray-400' ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                                    </svg>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-start justify-between mb-0.5">
                                        <h4 class="font-bold text-gray-800 text-sm"><?= esc($doctor['name']) ?></h4>
                                    ...
                                    <p class="text-xs text-gray-500 font-medium"><?= esc($doctor['specialization']) ?></p>
                                    <p class="text-xs text-gray-400 mt-0.5"><?= esc($doctor['experience_years']) ?> tahun pengalaman</p>
                                    <div class="flex items-center justify-between mt-3">
                                        <div class="flex items-center">
                                            <span class="w-2 h-2 <?= $doctor['status'] == 'online' ? 'bg-green-500 status-online' : 'bg-gray-300' ?> rounded-full mr-1.5"></span>
                                            <span class="text-xs <?= $doctor['status'] == 'online' ? 'text-green-600' : 'text-gray-400' ?> font-semibold"><?= ucfirst($doctor['status']) ?></span>
                                        </div>
                                        <span class="text-blue-600 font-bold text-sm">Rp <?= number_format($doctor['consultation_fee'], 0, ',', '.') ?></span>
                                    </div>
                                </div>
                            </div>
                ...
                <?php else: ?>
                    <div class="col-span-full py-10 text-center">
                        <p class="text-gray-500">Tidak ada dokter yang tersedia saat ini.</p>
                    </div>
                <?php endif; ?>
