...
<div id="history-detail-screen" class="flex flex-col h-full bg-[#f5f1ec] fade-in w-full">
    <!-- Header -->
    <div class="bg-[#ffffff] px-4 py-3 border-b border-[#d3cec6] flex items-center justify-between sticky top-0 z-10 ">
        <div class="flex items-center">
            ...
            <h1 class="text-base font-bold text-[#111111]">Riwayat Pasien</h1>
        </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-5 pb-20 mx-auto w-full max-w-4xl">
        ...
        <div class="bg-[#ffffff] rounded-[24px] p-5 border border-gray-50 mb-4 slide-up">
            <div class="flex justify-between items-start mb-4">
                <div class="flex items-center">
                    <div class="w-12 h-12 bg-green-50 rounded-[16px] flex items-center justify-center mr-4 text-[#111111] font-bold text-lg">
                        <?= substr($appointment['patient_name'], 0, 1) ?>
                    </div>
                    <div>
                        <h2 class="text-lg font-bold text-[#111111] mb-1"><?= esc($appointment['patient_name']) ?></h2>
                        <p class="text-sm text-[#626260] capitalize"><?= esc($appointment['gender'] ?? 'Pasien') ?></p>
                    </div>
                </div>
                <?php 
                ...
                ?>
                <span class="text-xs font-bold <?= str_replace('bg-', 'text-', $colorClass) ?> <?= str_replace('bg-', 'bg-', $colorClass) ?>/10 px-3 py-1.5 rounded-full capitalize mt-1">
                    <?= $appointment['status'] ?>
                </span>
            </div>
            
            <div class="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100 mt-2">
                <div>
                    <p class="text-xs text-[#7b7b78] mb-1">Tanggal Konsultasi</p>
                    <p class="text-sm font-semibold text-[#111111]"><?= date('d F Y', strtotime($appointment['appointment_date'])) ?></p>
                </div>
                <div>
                    <p class="text-xs text-[#7b7b78] mb-1">Waktu Selesai</p>
                    <p class="text-sm font-semibold text-[#111111]"><?= date('H:i', strtotime($appointment['time_slot'])) ?> WIB</p>
                </div>
                <div class="col-span-2">
                    <p class="text-xs text-[#7b7b78] mb-1">Keluhan Awal Pasien</p>
        ...
        <div class="bg-[#ffffff] rounded-[24px] p-5 border border-gray-50 mb-4 slide-up slide-up-delay-1">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <h4 class="text-xs font-semibold text-[#7b7b78] uppercase tracking-wider mb-2">Catatan Tambahan</h4>
                    <div class="p-3 bg-[#f9f8f6] rounded-[16px] border border-[#ebe7e1]">
        ...
        <!-- Action Buttons -->
        <div class="mt-6">
            <button onclick="window.location.href='<?= base_url('doctor/chat?id=' . $appointment['id']) ?>'" class="w-full py-3.5 bg-[#ffffff] text-[#111111] border border-[#d3cec6] rounded-[16px] text-sm font-bold hover:bg-gray-50 transition-all text-center">
                Lihat Log Chat Pasien Ini
            </button>
        </div>
    </div>
</div>
